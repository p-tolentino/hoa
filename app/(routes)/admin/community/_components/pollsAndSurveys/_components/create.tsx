...
function Create () {
  ...
  const { control, register, getValues, setValue, handleSubmit, formState: { errors }, reset } = form;
  
  const { fields: questionFields, append: appendQuest, remove: removeQuest } = useFieldArray({
    control,
    name: "questions"
  });
              ...
              <Stack spacing='15px'>
                ...
                {questionFields.map((question, qIndex) => (
                    ...
                    <HStack>
                      ...
                      <Box alignSelf='center' ml='2%'>
                        {qIndex === questionFields.length - 1 && (
                          <Button type="button" onClick={() => appendQuest({ text: '', options: [{ text: '' }] })}>Add Question</Button>
                        )}
                        {questionFields.length > 1 && (
                           <Button type="button" onClick={() => removeQuest(qIndex)}><DeleteIcon/></Button>
                        )}
                      </Box>
                      
                    </HStack>
                    
                    <Input {...register(`questions.${qIndex}.text`)} placeholder="Question" />
                  ...
                  <HStack key={oIndex} mt={2}>
                    ...
                    />
                    {oIndex >= 0 && ( 
                      <Button mt={2} onClick={() => appendOption(qIndex)}><AddIcon/></Button>
                    
                    )}
                    {oIndex > 0 && (
                      <Button onClick={() => removeOption(qIndex, oIndex)}><DeleteIcon /></Button>
                    )}
