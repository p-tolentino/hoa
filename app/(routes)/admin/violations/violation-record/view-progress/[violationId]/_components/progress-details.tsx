    ...
    <div>
      <Flex justifyContent='space-between'>
        <Flex gap={5}>
          ...
          <Badge
            className={cn(
              'text-md h-[min-content] py-2 px-4 text-center justify-center',
              tempViolation.status === 'For Review'
                ? 'bg-yellow-700'
                : tempViolation.status === 'Invalid'
                ? 'bg-red-800'
                : tempViolation.status === 'For Assignment'
                ? 'bg-yellow-800'
                : tempViolation.status === 'Pending Violation Letter'
                ? 'bg-orange-800'
                : tempViolation.status === 'Negotiating (Letter Sent)'
                ? 'bg-blue-900'
                : tempViolation.status === 'Closed; Penalty Fee in SOA'
                ? ''
                : tempViolation.status === 'Closed; Appealed'
                ? 'bg-green-700'
                : 'display-none'
            )}
          >
