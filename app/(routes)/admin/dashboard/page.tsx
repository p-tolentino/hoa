...
/* Card Functions */
import { getMemberCount, getHoaFunds,  getViolations} from "@/server/data/dashboard";
...
export default function ExampleChart() {
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        ...
        if (typeof fetchedFunds === 'number') { // Check if fetchedFunds is a number
        ...
        if (fetchedViolations) {
          ...
          const violationTypeCounts = fetchedViolations.reduce((acc: ViolationCounts, violation) => {
  ...
  const calculateFinancialSummary = (transactions: Transaction[] | null): MonthlySummary[] => {
    // If transactions is null, return an empty array immediately
    if (!transactions) {
      return [];
    }
  
    const summaryByMonth: SummaryByMonth = transactions.reduce((acc: SummaryByMonth, transaction: Transaction) => {
      const date = new Date(transaction.dateIssued);
      const yearMonthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; // Format: YYYY-MM
  
      // Initialize the month entry in accumulator if it doesn't exist
      if (!acc[yearMonthKey]) {
        acc[yearMonthKey] = { yearMonth: yearMonthKey, income: 0, expense: 0, net: 0};
      }
    ...
    const financialSummary: MonthlySummary[] = Object.entries(summaryByMonth).map(([yearMonth, { income, expense }]) => ({
      yearMonth,
      income,
      expense,
      net: income - expense,
        ...
        <Grid templateColumns="repeat(5, 1fr)" gap={6}>
          ...
          <GridItem colSpan={2}>
            <Card>
              ...
              <CardBody>
              <ViolationPieChart series={violationChartData.series} labels={violationChartData.labels} />
              </CardBody>
            </Card>
          </GridItem>
        </Grid>
